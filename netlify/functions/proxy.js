...
const BASE_URL = process.env.PROCORE_BASE_URL || 'https://us02.api.procore.com';
...
exports.handler = async (event) => {
  ...
  if (!endpoint) {
    return {
      statusCode: 400,
      headers,
      body: JSON.stringify({ error: 'No endpoint specified. Use ?endpoint=/rest/v1.0/...' })
    };
  }
  ...
  try {
    ...
    // Add Procore-Company-Id header if provided
    if (company_id) {
      requestHeaders['Procore-Company-Id'] = company_id;
    }

    // Make the request to Procore API
